...
const RestaurantStats = (props: {
	...
	const lockPageScroll = () => {
		...
		if (!isTouch) return;
		if (frozenScrollY.current !== null) return;
	...
	const unlockPageScroll = () => {
		const y = frozenScrollY.current;
		if (y === null) return;
		frozenScrollY.current = null;
		clearBodyLock();
		window.scrollTo(0, y);
	};
	useEffect(() => {
		return () => {
			if (frozenScrollY.current !== null)
				clearBodyLock();
		};
	}, []);
	...
	const overviewLayout = useMemo<Partial<Layout>>(
		() => ({
			title: {
				text:
					'Restaurants — average score vs visits' +
					suffix,
			},
	...
	const layout = useMemo<Partial<Layout>>(
		() => ({
			title: {
				text: selected
					? `${selected} — scores by reviewer (avg ${avg.toFixed(
							1,
					  )})`
					: 'Pick a restaurant',
			},
	...
	return (
		<div className='w-full flex flex-col space-y-[3vh]'>
			...
			<Autocomplete
				...
				defaultItems={restaurants.map((r) => ({
					key: r,
				}))}
